...
class retrieval_net(nn.Module):
    def __init__(self,cfg, is_train = True, is_transform=False): #
        ...
        if self.is_transform:
            self.Linear_fusing1 = nn.Sequential(nn.Linear(1000, 1024), nn.BatchNorm1d(1024), nn.LeakyReLU())
            self.Linear_fusing2 = nn.Sequential(nn.Linear(1000, 2048), nn.BatchNorm1d(2048), nn.LeakyReLU(), nn.Linear(2048,1024), nn.BatchNorm1d(1024),nn.LeakyReLU())
            self.attention_layer = TextEncoder()
        # if self.is_transform:
    ...
    def forward(self, images):
        ...
        if self.is_transform:
            fu1 = self.Linear_fusing1(features).unsqueeze(-1)
            fu2 = self.Linear_fusing2(features).unsqueeze(-1)
            em_features = torch.cat([fu1,fu2],dim=-1)
            embeded_feature = self.attention_layer(em_features)
